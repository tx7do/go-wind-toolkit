...
func generateTransportInfra(f *codegen.File, defaultHost string) {
	...
	f.P()
	if defaultHost != "" {
		f.P("const DEFAULT_HOST = ", tsSingleQuote(defaultHost), ";")
		f.P()
	}
	...
	f.P(t(2), "this.eventSource.addEventListener('message', (event) => {")
	f.P(t(3), "try {")
	...
	f.P(t(2), "this.socket.addEventListener('message', (event) => {")
	f.P(t(3), "try {")
	...
	f.P(t(1), "const baseUrl = opts?.baseUrl ?? (typeof DEFAULT_HOST === 'undefined' ? undefined : `https://${DEFAULT_HOST}`);")
